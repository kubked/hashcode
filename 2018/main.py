...
def solve(metadata, rides):
    ...
    for ride in sorted(rides, key=lambda r: -r.dist):
        car_assignments = []
        for car in cars:
...
def solve_2(metadata, rides):
    ...
    for i in range(metadata.F):
        car = Car(i)
        cars.append(car)
        ride_value = lambda r: ride_car_value(car, r)
        ride = max(rides, key=ride_value)
        while car.adding_value(ride, metadata.B) > 0:
            car.add(ride)
            rides.remove(ride)
            ride = max(rides, key=ride_value)
    return cars
...
if __name__ == "__main__":
    ...
    solution = solve_2(metadata, rides)
